...
export class CallStore extends AsyncStoreWithClient<EmptyObject> {
    ...
    protected async fetchTransports(): Promise<void> {
        ...
        // Reading it can be disabled via config so the app never falls back to the user's
        // `<server_name>/.well-known/matrix/client` endpoint; the modern endpoint above is unaffected.
