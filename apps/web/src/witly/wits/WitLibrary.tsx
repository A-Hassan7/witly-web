...
/**
 * WitLibrary — the embeddable Wit Library surface.
 *
 ...
 *
 * This is presentational and prop-free so it can be rendered both by the
 * standalone space-panel launcher (via `WitLibraryDialog`) and as the Witly
 * panel "Wits" tab (P5). The reusable {@link CreateWitForm} is exported so the
 * Suggestions tab can offer inline custom-Wit creation.
 */
...
export function WitLibrary(): JSX.Element {
...
export interface CreateWitFormProps {
    onCancel: () => void;
    onSaved: () => void;
    /** Compact layout for embedding inside the Suggestions tab. */
    compact?: boolean;
}

export function CreateWitForm({ onCancel, onSaved, compact = false }: CreateWitFormProps): JSX.Element {
    ...
    return (
        <div className={`witly_WitForm${compact ? " witly_WitForm--compact" : ""}`}>
