...
| 5 | `apps/web/src/components/views/dialogs/UserTab.ts` + `apps/web/src/components/views/dialogs/UserSettingsDialog.tsx` | `UserTab.ts`: add `Witly = "USER_WITLY_TAB"` as the first enum member. `UserSettingsDialog.tsx`: import `WitlyUserSettingsTab` + a `FavouriteIcon`, `tabs.push(new Tab(UserTab.Witly, "Witly", <FavouriteIcon/>, <WitlyUserSettingsTab/>))` as the FIRST push in `getTabs()` (top of the list), and add `case UserTab.Witly: return "Witly";` to `titleForTabID`. All added lines marked `WITLY SEAM`. | The Witly account section (profile, plan, credits, connected apps, delete) must live at the top of Element's User Settings dialog. No module-API hook adds a settings tab. Purely additive — reverting these lines (and removing `witly/`) restores vanilla Element settings. | active |
...
| Account settings (P6) | `apps/web/src/witly/account/*` | Prop-free `WitlyUserSettingsTab.tsx` mounted via seam #5 as the top tab of Element User Settings: Profile (editable Matrix display name + avatar, read-only Supabase email/phone, Element sign-out), Plan (stubbed free-trial card + disabled Upgrade), AI credits (usage bar + reset period from `/ai/credits`), Connected apps (from `getAllBridges`, "Manage" reuses `openConnectDialog`), Danger zone (type-`DELETE` → `deleteAccount`). `account.pcss` (brand + Compound tokens). Additive. |
...
| `apps/web/src/witly/element/profile.ts` (P6) | `MatrixClientPeg` (`../../MatrixClientPeg`) + `dispatcher` (`../../dispatcher/dispatcher`) | **WRITE seam.** Backs the account tab. `getMatrixProfile` (read display name + avatar), `setMatrixDisplayName`, `setMatrixAvatarFromFile` (upload → `setAvatarUrl`), and `signOut` (best-effort Supabase sign-out + `witlySession.clear()` + Element's `defaultDispatcher.dispatch({action:"logout"})` — identical to Element's native profile sign-out). The ONLY Witly file that writes Matrix profile / triggers logout. Single-file fix if internals move. |
